...
for i = 1:40
    ...
    train_a = a4(:,5*i-4:5*i);
    test_a = a4;
    test_a(:,5*i-4:5*i) = [];
    mu_a = mean(train_a,2);
    sigma_a = cov(train_a');
    
    train_b = b4(:,5*i-4:5*i);
    test_b = b4;
    test_b(:,5*i-4:5*i) = [];
    mu_b = mean(train_b,2);
    sigma_b = cov(train_b');
...
%% calculate errors
error_4_med = mean(error_med)
std_4_med = std(error_med)

error_4_ged = mean(error_ged)
std_4_ged = std(error_ged)

error_4_1nn = mean(error_1nn)
std_4_1nn = std(error_1nn)

error_4_3nn = mean(error_3nn)
std_4_3nn = std(error_3nn)

error_4_5nn = mean(error_5nn)
std_4_5nn = std(error_5nn)