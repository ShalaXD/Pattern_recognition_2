...
for i = 1:40
    ...
    train_a = a2(:,5*i-4:5*i);
    test_a = a2;
    test_a(:,5*i-4:5*i) = [];
    mu_a = mean(train_a,2);
    sigma_a = cov(train_a');
    
    train_b = b2(:,5*i-4:5*i);
    test_b = b2;
    test_b(:,5*i-4:5*i) = [];
    mu_b = mean(train_b,2);
    sigma_b = cov(train_b');
...
%% calculate errors
error_2_med = mean(error_med)
std_2_med = std(error_med)

error_2_ged = mean(error_ged)
std_2_ged = std(error_ged)

error_2_1nn = mean(error_1nn)
std_2_1nn = std(error_1nn)

error_2_3nn = mean(error_3nn)
std_2_3nn = std(error_3nn)

error_2_5nn = mean(error_5nn)
std_2_5nn = std(error_5nn)