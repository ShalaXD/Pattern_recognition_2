...
for i = 1:40
    ...
    train_a = a3(:,5*i-4:5*i);
    test_a = a3;
    test_a(:,5*i-4:5*i) = [];
    mu_a = mean(train_a,2);
    sigma_a = cov(train_a');
    
    train_b = b3(:,5*i-4:5*i);
    test_b = b3;
    test_b(:,5*i-4:5*i) = [];
    mu_b = mean(train_b,2);
    sigma_b = cov(train_b');
...
%% calculate errors
error_3_med = mean(error_med)
std_3_med = std(error_med)

error_3_ged = mean(error_ged)
std_3_ged = std(error_ged)

error_3_1nn = mean(error_1nn)
std_3_1nn = std(error_1nn)

error_3_3nn = mean(error_3nn)
std_3_3nn = std(error_3nn)

error_3_5nn = mean(error_5nn)
std_3_5nn = std(error_5nn)